...
export default function Header() {
  ...
  const handlesubmit = (evt) => {
    evt.preventDefault();
    axios.post("https://todos-backend-production-6961.up.railway.app/task", {
      task,
      date,
      complete,
    });
  ...
  return (
    <>
      ...
      <div id="links">
        <Link to="/" className="link def" onClick={alll}>
          All
        </Link>
        <Link to="pending" className="link" onClick={pend}>
          Pending
        </Link>
        <Link to="complete" className="link" onClick={comp}>
          Complete
        </Link>
      </div>
    </>
